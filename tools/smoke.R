...
need <- c("httr2","jsonlite","tibble","dplyr","purrr","stringr","memoise","cachem","rappdirs","stringi")
...
try({
  itms <- nso_itms()
  cat("SMOKE: nso_itms() => rows=", nrow(itms), " cols=", ncol(itms), "\n", sep = "")
}, silent = TRUE)
...
if (requireNamespace("sf", quietly = TRUE)) {
  try({
    ...
    cat("SMOKE: mn_boundaries(ADM1) => features=", nrow(g), " epsg=", epsg, "\n", sep = "")
  }, silent = TRUE)
} else {
