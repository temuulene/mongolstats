...
mn_boundaries_normalize <- function(g, name_col = "shapeName") {
  if (!name_col %in% names(g)) return(g)
...
mn_join_by_name <- function(data, name_col, level = "ADM1", boundaries = NULL) {
  if (is.null(boundaries)) boundaries <- mn_boundaries(level)
...
#' @export
mn_fuzzy_join_by_name <- function(data, name_col, level = "ADM1", boundaries = NULL,
                                  max_distance = 2, method = c("osa", "lv", "jw", "dl")) {
  ...
  if (!length(keys_d) || !length(keys_g)) return(dplyr::left_join(boundaries, d, by = "name_std"))
  ...
  map_df <- tibble::tibble(name_std = keys_d,
                           match_std = keys_g[best_idx],
                           dist = as.numeric(best_dst))
  map_df <- map_df[map_df$dist <= max_distance | map_df$name_std == map_df$match_std, , drop = FALSE]
