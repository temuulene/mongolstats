...
test_that("nso_data works with multiple values", {
  ...
  result <- tryCatch({
    nso_data(
      tbl_id = "DT_NSO_0300_001V2",
      selections = list(Sex = c("Male", "Female"), Age = "Total", Year = "2024"),
      labels = "en"
    )
  }, error = function(e) NULL)
...
test_that("nso_table_periods returns valid periods", {
  ...
  periods <- tryCatch(nso_table_periods("DT_NSO_0300_001V2"), error = function(e) NULL)
...
test_that("nso_package works with multiple tables", {
  ...
  result <- tryCatch(nso_package(reqs, labels = "none"), error = function(e) NULL)
