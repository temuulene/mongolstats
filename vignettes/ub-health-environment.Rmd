...
> `r nrow(cor_input)` districts with complete data. With so few observations
...
1.  **Density & Infant Mortality**: Population density and Infant Mortality Rate (IMR) show a positive correlation (r = `r round(cor_mat["Density", "IMR"], 2)`) in this small sample. Denser districts, which often include crowded ger areas with limited infrastructure, *may* face higher infant-health risks—but the caveat above should temper any causal reading.
2.  **Respiratory Health**: Respiratory mortality correlates only weakly with density (r = `r round(cor_mat["Resp. Rate", "Density"], 2)`) and with IMR (r = `r round(cor_mat["Resp. Rate", "IMR"], 2)`). This may reflect city-wide pollution exposure that is relatively uniform across districts, or that mortality data captures only the most severe outcomes, masking broader morbidity.
