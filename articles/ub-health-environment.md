...
get_ub_shapes <- function() {
  ...
  # Spatial filter (turn off S2 for robustness, then restore the caller's
  # setting on exit so this helper does not leave a global side effect behind)
...
> **Read these correlations with caution.** They are computed across
> only 9 districts with complete data. With so few observations and no
> significance testing, the coefficients are statistically unstable—a
> single district can swing them substantially—and confidence intervals
> would be very wide. They also describe *district-level averages*, so
> they cannot be used to infer individual-level risk (the ecological
> fallacy). Treat the patterns below as hypothesis-generating, not
> confirmatory.

1.  **Density & Infant Mortality**: Population density and Infant
    Mortality Rate (IMR) show a positive correlation (r = 0.85) in this
    small sample. Denser districts, which often include crowded ger
    areas with limited infrastructure, *may* face higher infant-health
    risks—but the caveat above should temper any causal reading.
2.  **Respiratory Health**: Respiratory mortality correlates only weakly
    with density (r = -0.14) and with IMR (r = 0.05). This may reflect
    city-wide pollution exposure that is relatively uniform across
    districts, or that mortality data captures only the most severe
    outcomes, masking broader morbidity.
3.  **Infrastructure Load**: The student-per-school ratio shows weak
    correlations with the other indicators, suggesting it is not a
    primary driver of the observed differences in this dataset.

## 5. Air Quality Monitoring Network

Ulaanbaatar’s air quality monitoring network consists of 18 stations
spread across 6 of the city’s 9 districts. Understanding their spatial
...
> **Bayanzurkh** has the most stations (5), reflecting its large area
> and population, followed by **Songinokhairkhan** and **Sukhbaatar** (4
> each). **Khan-Uul** and **Chingeltei** have 2 stations each, and
> **Bayangol** has a single station. Three of Ulaanbaatar’s nine
> districts—**Baganuur**, **Bagakhangai**, and **Nalaikh**—have no
> station in this network at all. This uneven distribution leaves clear
> spatial gaps, particularly in the sparsely monitored peripheral ger
> districts where exposure may be highest.
