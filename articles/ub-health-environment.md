...
p_dens <- ggplot(ub_pop_map) +
    ...
    labs(title = "Population Density by District (2024)",
...
p_trend <- pop_trend |>
    ...
    labs(title = "Population Growth Trends (2013-2024)",
...
p_edu <- ggplot(edu_load, aes(x = reorder(District, Students_Per_School), 
    ...
    coord_flip() +
    scale_y_continuous(labels = scales::comma) +
    labs(title = "Educational Load: Students per School (2024)",
...
p_resp <- ggplot(health_metrics, aes(x = reorder(District, Resp_Death_Rate), 
    ...
    coord_flip() +
    labs(title = "Respiratory Mortality Rate (2024)",
...
p_imr <- ggplot(imr_data, aes(x = reorder(District, value), y = value,
    ...
    coord_flip() +
    labs(title = "Infant Mortality Rate (2023)",
...
# Correlation Matrix
cor_mat <- analysis_df |>
    select(Density, `Students/School` = Students_Per_School, 
...
p_cor <- ggplot(cor_long, aes(x = Var1, y = Var2, fill = Correlation,
    ...
    labs(title = "Correlation of District Indicators", 
...
p_map <- ggplot() +
  geom_sf(data = ub_shapes, aes(text = paste0("<b>", District, "</b>")), 
          fill = "grey90", color = "white", linewidth = 0.5) +
  geom_sf(data = ub_stations_sf, 
          aes(text = paste0("<b>", Station, "</b><br>",
                            "District: ", District, "<br>",
                            "Source: ", Source)),
          color = "#e74c3c", size = 3, alpha = 0.8) +
  geom_sf_text(data = ub_shapes, aes(label = District), 
...
p_coverage <- ggplot(station_coverage, 
                     aes(x = reorder(District, Pop_per_Station), 
  ...
  geom_col() +
  coord_flip() +
  scale_fill_manual(values = c("High Coverage" = "#27ae60", 
